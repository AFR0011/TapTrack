...
export const CATEGORY_ICON_IDS = CATEGORY_ICON_OPTIONS.map(([id]) => id);
export const CATEGORY_COLOR_VALUES = CATEGORY_COLOR_OPTIONS.map(({ value }) => value);

export function isCategoryIconId(value: unknown): value is CategoryIconId {
  return typeof value === 'string' && CATEGORY_ICON_IDS.some((id) => id === value);
}

export function isCategoryColor(value: unknown): value is (typeof CATEGORY_COLOR_VALUES)[number] {
  return typeof value === 'string' && CATEGORY_COLOR_VALUES.some((color) => color === value);
}
