...
export function QuickAddTransaction({ prefill, onSaved }: { prefill?: QuickAddPrefill; onSaved?: () => void }) {
  ...
  useEffect(() => {
    let active = true;
    void getSignedInEmail().then((email) => { if (active) setAccountSignedIn(Boolean(email)); });
    return () => { active = false; };
  }, []);
  ...
  const currency = currencyOverride ?? settings?.defaultCurrency ?? activeCurrencies[0] ?? 'TRY';
  const typedCategories = useMemo(() => (categories ?? []).filter((category) => category.type === type), [categories, type]);
  const inferredCategory = useMemo(() => findCategoryForTransaction(categories ?? [], type, title), [categories, title, type]);
  const aiActive = Boolean(settings?.aiCategorizationEnabled && accountSignedIn);
  const aiSuggestion = useAICategorySuggestion({ title, type, categories: categories ?? [], enabled: aiActive, blocked: categoryManuallyChosen });
  const manualCategoryValid = typedCategories.some((category) => category.id === categoryId);
  const aiCategoryValid = typedCategories.some((category) => category.id === aiSuggestion.categoryId);
  ...
  if (!categories || !settings || !balances) {
    return <section className="rounded-2xl border border-subtle bg-surface p-5" aria-busy="true"><div className="h-36 animate-pulse rounded-xl bg-surface-muted" aria-hidden="true" /></section>;
  }
