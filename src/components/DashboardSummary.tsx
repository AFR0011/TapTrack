...
export default function DashboardSummary() {
  ...
  const dashboardTransactions = useMemo(
    () =>
      (transactions ?? []).filter(
        (transaction) =>
          transaction.date >= thirtyDaysAgo || transaction.date.startsWith(currentMonth)
      ),
  ...
  const topCategories = [...categorySpending.entries()]
    .map(([categoryId, amount]) => ({
      name: categoryById.get(categoryId) ?? 'Other',
      amount,
    }))
      ...
      <div className="grid gap-4 lg:grid-cols-2">
          ...
          {budgetAvailable > 0 ? (
            <>
              <div className="mt-4 flex items-end justify-between gap-4">
                ...
                >
                  {formatCurrency(budgetRemaining, budgetCurrency)} left
                </p>
              </div>
              <ProgressBar
                className="mt-4"
        ...
        <section className="rounded-2xl border border-subtle bg-surface p-5">
          ...
          {topCategories.length ? (
            <div className="mt-4 space-y-3">
              {topCategories.map((item, index) => (
                <div key={item.name} className="flex items-center gap-3">
                  <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-surface-muted text-xs font-bold text-muted">
                    {index + 1}
                  </span>
            ...
            <p className="mt-5 text-sm text-muted">No spending yet this month.</p>
          )}
        </section>
      </div>
    ...
    <div className="rounded-2xl border border-subtle bg-surface p-4">
      ...
      <p className={`mt-2 text-xl font-bold tabular-nums ${toneClass}`}>
        {loading ? (
          ...
          <AnimatedNumber value={value} format={(amount) => formatCurrency(amount, currency)} />
        )}
      </p>
    </div>
  );
  ...
  const codes = [
    ...new Set(
      balances
        ...
        .filter((currency) => currency !== quote)
    ),
  ];
