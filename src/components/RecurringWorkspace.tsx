...
export default function RecurringWorkspace() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setForm((current) => ({ ...current, amount: '', title: '' }));
      setStatus('Recurring transaction saved.');
    } catch (err) {
      setStatus(err instanceof Error ? err.message : 'Recurring transaction could not be saved.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const runDueCheck = async () => {
    ...
    try {
      ...
      setStatus(`Created ${result.created}, skipped ${result.skipped}, failed ${result.failed}.`);
    } catch (err) {
      setStatus(err instanceof Error ? err.message : 'Due recurring check failed.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <h2 className="text-base font-semibold text-slate-950">New recurring transaction</h2>
        ...
        <button type="button" onClick={handleCreate} disabled={saving} className="mt-4 rounded-md bg-blue-500 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-600 disabled:opacity-60">
