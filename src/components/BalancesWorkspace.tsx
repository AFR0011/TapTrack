...
import type { Balance, BalanceCheckpoint, Currency } from '@/types';
...
export default function BalancesWorkspace() {
  ...
  const { activeGroups, archivedGroups } = useMemo(() => {
    ...
    return {
      activeGroups: groupBalances((balances ?? []).filter((balance) => active.has(balance.currency))),
      archivedGroups: groupBalances((balances ?? []).filter((balance) => !active.has(balance.currency))),
    };
  }, [activeCurrencies, balances]);

  if (balances === undefined || settings === undefined || adjustments === undefined) {
    return (
    ...
    <div className="space-y-5">
      ...
      <section aria-labelledby="current-balances-title">
        <div className="mb-3 flex items-end justify-between gap-4">
          <div>
            <h2 id="current-balances-title" className="text-sm font-semibold text-primary">Active currencies</h2>
            <p className="mt-0.5 text-xs font-medium text-muted">These currencies are available for new activity.</p>
          </div>
        </div>
        {activeGroups.length === 0 ? (
          ...
          <BalanceGrid groups={activeGroups} />
        )}
      </section>
        ...
        <details className="group rounded-2xl border border-subtle bg-surface">
          ...
          >
            <div className="min-w-0 flex-1">
              <h2 className="text-sm font-semibold text-primary">Archived currency balances</h2>
              <p className="mt-0.5 text-xs font-medium text-muted">
                {archivedGroups.length} currenc{archivedGroups.length === 1 ? 'y' : 'ies'} kept for financial history
              </p>
            </div>
          ...
          <div className="border-t border-subtle p-4">
            <p className="mb-4 text-xs font-medium text-muted">
              Archived currencies stay in past transactions and balances but are not offered for new entries.
            </p>
            <BalanceGrid groups={archivedGroups} muted />
          </div>
        </details>
      ) : null}
...
function groupBalances(balances: Balance[]): BalanceGroup[] {
...
function BalanceGrid({ groups, muted = false }: { groups: BalanceGroup[]; muted?: boolean }) {
  return (
    <div className="grid gap-4 md:grid-cols-2">
      {groups.map(([currency, rows]) => {
        const total = rows.reduce((sum, row) => sum + row.amount, 0);
        return (
          <article key={currency} className={cn('rounded-2xl border border-subtle bg-surface p-5', muted && 'bg-surface-muted')}>
