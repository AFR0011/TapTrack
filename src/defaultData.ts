...
import { SUPPORTED_CURRENCIES, SUPPORTED_METHODS } from '@/types';
...
export function createDefaultCategories(now = new Date().toISOString()): Category[] {
  return DEFAULT_CATEGORY_TEMPLATES.map((category) => ({
    ...category,
    createdAt: now,
    updatedAt: now,
  }));
...
export function createInitialBalances(now = new Date().toISOString()): Balance[] {
...
export function findCategoryForTransaction(
  ...
  if (type === 'income') {
    return typedCategories.find((category) => category.name === 'Income') ?? typedCategories[0];
  }
  ...
  return (
    typedCategories.find((category) => category.name === 'Other') ??
