...
const createCaptureToken = vi.fn(() => 'ttcap_test_private_key');
const hashCaptureToken = vi.fn(() => 'a'.repeat(64));
...
describe('POST /api/capture-tokens', () => {
  ...
  it('refuses to mint a key until the canonical account ledger is ready', async () => {
    ...
    expect(createCaptureToken).not.toHaveBeenCalled();
  });
  ...
  it('fails closed when canonical readiness cannot be checked', async () => {
    ...
    adminFrom.mockReturnValue(settings);
  ...
  it('mints a one-time raw key only after canonical setup is ready', async () => {
    ...
    expect(hashCaptureToken).toHaveBeenCalledWith('ttcap_test_private_key');
